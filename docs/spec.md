...
`table`, `form`, `select`, `textarea`, `video`, `audio`, `iframe`,
`canvas` は v0.1 では禁止。
...
通常の CSS は使わない。代わりに design tokens を TOML/JSON で定義する。
...
gpuiHTML はこれを参照して `bg-surface`, `rounded-lg`, `gap-3` を解決する。
`bg-<token>` で `<token>` が tokens TOML に存在しない場合は compile error。
...
- **crates.io naming.** workspace 内に `gpui-rsx` crate を含むと、上述の
  既存 crate と名前衝突するため publish できない。リポジトリ名の通り
  `gpui-html-core` 等に rename することを別途検討する。
- **theme tokens の正準形。** TOML / JSON / Rust const のどれを source of
  truth にするか、また dark/light mode の切り替えはどう表現するかは
  v0.2 で決める。
